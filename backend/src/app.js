...
<head>
  ...
  <style>
    ...
    table { width:100%; min-width:1120px; border-collapse:collapse; margin-top:14px; }
...
<body>
  <div class="wrap">
    <section class="hero">
      ...
      <p>Güvenli demo paneli. v0.3 Smart Radar momentum, likidite, 24 saat aralık konumu, spread riski, pump/dump kovalamama ve yön eğilimi okur. Gerçek emir hâlâ kilitlidir.</p>
      <div class="grid">
    ...
    <div class="grid">
      ...
      <div class="card"><h2>Güvenlik Durumu</h2><p>Gerçek emir endpointi kilitlidir. API key olmadan sadece public veri okunur. Live trading sonradan server/VPS aşamasında ayrıca açılır.</p></div>
    </div>
...
function money(value) { if (value === null || value === undefined || Number.isNaN(Number(value))) return '-'; return Number(value).toLocaleString('en-US', { maximumFractionDigits: 6 }); }
...
async function loadScanner() {
  ...
  const rows = (data.results || []).map((item) => {
    ...
    const binance = tickers.find((x) => x.exchange === 'binance') || {};
    ...
    return '<tr>' +
      ...
      '<td>' + pct(m.avgRangePosition !== null && m.avgRangePosition !== undefined ? m.avgRangePosition * 100 : null) + '</td>' +
  ...
  showSmall('Smart Radar v0.3 özeti: ' + (data.count || 0) + ' coin tarandı.', (data.results || []).map(item => ({ symbol:item.symbol, score:item.score, confidence:item.confidence, risk:item.riskTier, yon:item.directionBias, karar:item.decision?.label, action:item.decision?.action, neden:item.decision?.reason, warnings:item.warnings })));
}
async function openPaperDemo() {
  await api('/paper/open', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ exchange:'binance', symbol:'BTCUSDT', side:'long', entryPrice:100000, stopLoss:99000, takeProfit:102000, positionSizeEur:100 }) });
  await loadStatus();
}
...
app.get('/health', async (req, res) => {
  res.json({ ok: true, service: 'Cidentia Market Intelligence & Risk Engine', mode: config.tradingMode, realTradingGateOpen: realTradingGateIsOpen(), timestamp: new Date().toISOString() });
});
...
app.get('/scanner', async (req, res) => {
  const symbols = req.query.symbols ? String(req.query.symbols).split(',') : DEFAULT_SCANNER_SYMBOLS;
...
app.post('/risk/evaluate', (req, res) => { res.json(evaluateTradeRisk(req.body || {})); });
app.get('/paper', (req, res) => { res.json(getPaperState()); });
app.post('/paper/open', (req, res) => { res.json(openPaperTrade(req.body || {})); });
app.post('/paper/reset', (req, res) => { res.json(resetPaperState()); });
app.post('/order/live', (req, res) => {
  res.status(403).json({ accepted: false, reason: 'LIVE_ORDER_ENDPOINT_LOCKED_IN_SAFE_FOUNDATION_VERSION', message: 'Use paper trading first. Live execution is intentionally blocked.' });
});

app.use((error, req, res, next) => { res.status(500).json({ ok: false, error: error.message }); });
