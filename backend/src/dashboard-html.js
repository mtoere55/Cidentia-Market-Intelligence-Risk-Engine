...
<head>
  ...
  <style>
    ...
    .workspace { display:grid; grid-template-columns:minmax(0,1fr) 330px; gap:12px; margin-top:12px; align-items:start; }
...
<body>
  <div class="wrap">
    <section class="topbar">
      <div class="brandLine">
        <div><h1>Cidentia Broker Ops Terminal</h1><p>v1.1 Paper Performance — sinyal kuyruğu, performans takibi, PnL, TP/Stop ve kompakt teknik ekran.</p></div>
...
function renderPaperSetupsHtml() { const setups = paperPerformance?.setups || []; if (!setups.length) return '<p>Paper setup kuyruğu boş. Bir satır seçip Queue Paper Setup bas.</p>'; return '<div class="setupList">' + setups.slice(0, 20).map(s => '<div class="setupItem"><b>'+s.symbol+'</b> | '+s.side+' | '+s.status+' | PnL '+eur((s.realizedPnlEur || 0) + (s.unrealizedPnlEur || 0))+'<br>Entry '+(s.entryPrice || (s.entryZone?.from + ' - ' + s.entryZone?.to))+' | Stop '+s.stopLoss+' | TP1 '+s.takeProfit1+'</div>').join('') + '</div>'; }
...
function renderPerformance(data) { paperPerformance = data; const s = data?.summary || {}; const total = Number(s.totalPnlEur || 0); const cls = total >= 0 ? 'safe' : 'danger'; document.getElementById('paperPnl').textContent = eur(total); document.getElementById('paperPnl').className = 'value ' + cls; document.getElementById('winRate').textContent = (s.winRate || 0) + '%'; document.getElementById('paperPerformance').innerHTML = '<div class="ticketRow"><span>Queued</span><b>'+(s.queued||0)+'</b></div><div class="ticketRow"><span>Active</span><b>'+(s.active||0)+'</b></div><div class="ticketRow"><span>TP1 / TP2</span><b>'+(s.tp1Hit||0)+' / '+(s.tp2Hit||0)+'</b></div><div class="ticketRow"><span>Stopped</span><b>'+(s.stopped||0)+'</b></div><div class="ticketRow"><span>Win Rate</span><b>'+(s.winRate||0)+'%</b></div><div class="ticketRow"><span>Realized</span><b>'+eur(s.realizedPnlEur||0)+'</b></div><div class="ticketRow"><span>Unrealized</span><b>'+eur(s.unrealizedPnlEur||0)+'</b></div><div class="ticketRow"><span>Total</span><b>'+eur(s.totalPnlEur||0)+'</b></div><button onclick="refreshPerformance()">Refresh Performance</button>'; if (activeTab === 'paper') renderTabs(); }
