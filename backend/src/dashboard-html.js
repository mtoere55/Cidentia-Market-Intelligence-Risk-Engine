...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    button.secondary { background:#334155; } button.dangerBtn { background:#9f1239; } button.soundBtn { background:#0f766e; } button.paperBtn { background:#16a34a; }
...
<body>
  <div class="wrap">
    <section class="topbar">
      <div class="brandLine">
        <div><h1>Cidentia Broker Ops Terminal</h1><p>v1.2 Price Map — fiyat şu an nerede, girişe ne kadar uzak, stop/kar yolu ne açıkça gösterilir.</p></div>
      ...
      <div id="signalBanner" class="signalStrip">
        <div><div class="signalTitle" id="signalTitle">SIGNAL MONITOR: STANDBY</div><div class="muted" id="signalText">Güçlü setup çıkarsa fiyat haritası ve sesli uyarı verir. Gerçek emir yok.</div></div>
...
function buildPriceMap(item) {
  ...
  let text = '';
  if (inZone) {
    status = 'giriş bölgesinde';
    statusClass = 'ready';
    entryTarget = current;
    text = 'Fiyat şu anda giriş bölgesinin içinde. Paper setup seçilmişse izlenebilir.';
  } else if (side === 'short') {
    if (current > high) { status = 'girişe düşmesi lazım'; entryTarget = high; text = 'Short için fiyatın giriş bölgesine doğru aşağı gelmesi beklenir.'; }
    else { status = 'giriş altına kaçmış'; entryTarget = low; text = 'Fiyat giriş bölgesinin altında. Short için hareket başlamış olabilir; kovalama riski var.'; statusClass = 'avoid'; }
  } else if (side === 'long') {
    if (current < low) { status = 'girişe yükselmesi lazım'; entryTarget = low; text = 'Long için fiyatın giriş bölgesine doğru yukarı gelmesi beklenir.'; }
    else { status = 'giriş üstüne kaçmış'; entryTarget = high; text = 'Fiyat giriş bölgesinin üstünde. Long için kovalama riski var.'; statusClass = 'avoid'; }
  }
  return {
    current,
    side,
    ...
    entryLow: low,
    entryHigh: high,
    toEntry: inZone ? 0 : signedDistancePct(current, entryTarget),
    toStop: signedDistancePct(current, Number(p.stopLoss)),
    toTp1: signedDistancePct(current, Number(p.takeProfit1)),
    toTp2: signedDistancePct(current, Number(p.takeProfit2)),
    text,
  };
}
...
function renderPaperSetupsHtml() { const setups = paperPerformance?.setups || []; if (!setups.length) return '<p>Paper setup kuyruğu boş. Bir satır seçip Queue Paper Setup bas.</p>'; return '<div class="setupList">' + setups.slice(0, 20).map(s => '<div class="setupItem"><b>'+s.symbol+'</b> | '+s.side+' | '+s.status+' | Current '+money(s.currentPrice)+' | PnL '+eur((s.realizedPnlEur || 0) + (s.unrealizedPnlEur || 0))+'<br>Entry '+(s.entryPrice || (s.entryZone?.from + ' - ' + s.entryZone?.to))+' | Stop '+s.stopLoss+' | TP1 '+s.takeProfit1+'</div>').join('') + '</div>'; }
...
function renderPriceMap(item) {
  const pm = buildPriceMap(item);
  const p = item.virtualTradePlan || {};
  document.getElementById('priceMapBox').innerHTML =
    '<div class="ticketRow"><span>Şu anki fiyat</span><b>'+money(pm.current)+'</b></div>'+
    '<div class="ticketRow"><span>Yön</span><b>'+pm.side+'</b></div>'+
    '<div class="ticketRow"><span>Durum</span><b><span class="pill '+pm.statusClass+'">'+pm.status+'</span></b></div>'+
    '<div class="ticketRow"><span>Girişe uzaklık</span><b>'+absPct(pm.toEntry)+'</b></div>'+
    '<div class="ticketRow"><span>Stop uzaklığı</span><b>'+absPct(pm.toStop)+'</b></div>'+
    '<div class="ticketRow"><span>Kar 1 uzaklığı</span><b>'+absPct(pm.toTp1)+'</b></div>'+
    '<div class="pathLine"><div class="pathBox"><b>NOW</b><br>'+money(pm.current)+'</div><div class="pathBox"><b>ENTRY</b><br>'+money(pm.entryLow)+' - '+money(pm.entryHigh)+'</div><div class="pathBox"><b>TP1 / STOP</b><br>'+money(p.takeProfit1)+' / '+money(p.stopLoss)+'</div></div>'+
    '<p class="muted">'+pm.text+'</p>';
}
function selectTicket(index) { selectedIndex = index; const item = lastResults[index]; if (!item) return; const p = item.virtualTradePlan || {}; const mm = item.moneyManagement || {}; const adv = item.operatorAdvice || {}; const pm = buildPriceMap(item); const zone = p.entryZone ? money(p.entryZone.from)+' - '+money(p.entryZone.to) : '-'; renderPriceMap(item); document.getElementById('executionTicket').innerHTML = '<div class="ticketRow"><span>Symbol</span><b>'+item.symbol+'</b></div><div class="ticketRow"><span>Current</span><b>'+money(pm.current)+'</b></div><div class="ticketRow"><span>Decision</span><b>'+ (adv.label || '-') +'</b></div><div class="ticketRow"><span>Bias</span><b>'+item.directionBias+'</b></div><div class="ticketRow"><span>Entry</span><b>'+zone+'</b></div><div class="ticketRow"><span>Stop</span><b>'+money(p.stopLoss)+'</b></div><div class="ticketRow"><span>TP1 / TP2</span><b>'+money(p.takeProfit1)+' / '+money(p.takeProfit2)+'</b></div><div class="ticketRow"><span>R/R</span><b>'+ (p.riskRewardToTp1 ?? '-') +'</b></div><div class="ticketRow"><span>Size</span><b>'+ (mm.enabled ? eur(mm.suggestedPositionEur) : '-') +'</b></div><button class="paperBtn" onclick="queueSelectedSetup()">Queue Paper Setup</button><p class="muted">PAPER ONLY. Gerçek emir yok.</p>'; const trend = item.trendStrength || item.metrics?.trendStrength || {}; const whale = item.whaleRisk || item.metrics?.whaleRisk || {}; document.getElementById('riskSnapshot').innerHTML = '<div class="ticketRow"><span>Trend</span><b>'+ (trend.label || '-') +'</b></div><div class="ticketRow"><span>Whale</span><b>'+ (whale.level || '-') +'</b></div><div class="ticketRow"><span>Liquidity</span><b>'+ (item.metrics?.liquidityScore ?? '-') +'</b></div><div class="ticketRow"><span>Risk Tier</span><b>'+ item.riskTier +'</b></div><div class="ticketRow"><span>24h</span><b>'+ pct(item.metrics?.avgChange) +'</b></div>'; }
